...
const HabLandingPage = () => {
  const { nextStep, loadQuote } = useHab();
  const [hoveredRow, setHoveredRow] = React.useState(null);

  const styles = {
    container: {
      minHeight: '100vh', padding: '2rem', backgroundColor: '#f8f9fa',
    ...
    title: { fontSize: '2rem', fontWeight: '700', color: '#0a1e3d', margin: '0 0 0.25rem 0' },
    subtitle: { fontSize: '1rem', color: '#666', margin: 0 },
    newQuoteBtn: {
      padding: '0.75rem 1.5rem', fontSize: '0.95rem', fontWeight: '600',
      color: '#ffffff', backgroundColor: '#0a1e3d', border: 'none',
    ...
    tableTitle: { fontSize: '1.1rem', fontWeight: '600', color: '#0a1e3d', margin: 0 },
    quoteCount: { fontSize: '0.85rem', color: '#888' },
    table: { width: '100%', borderCollapse: 'collapse' },
    th: {
      textAlign: 'left', padding: '0.75rem 1.5rem', fontSize: '0.75rem',
      fontWeight: '600', color: '#888', textTransform: 'uppercase',
      letterSpacing: '0.05em', borderBottom: '1px solid #e9ecef', backgroundColor: '#fafbfc',
    },
    td: { padding: '0.85rem 1.5rem', fontSize: '0.9rem', color: '#333', borderBottom: '1px solid #f0f0f0' },
    tr: { cursor: 'pointer', transition: 'background-color 0.15s ease' },
    idCell: { fontWeight: '600', color: '#0a1e3d' },
    statusBadge: (status) => ({
    ...
    openBtn: {
      padding: '0.35rem 0.8rem', fontSize: '0.8rem', fontWeight: '500',
      color: '#0a1e3d', backgroundColor: 'transparent', border: '1.5px solid #0a1e3d',
      borderRadius: '4px', cursor: 'pointer', transition: 'all 0.15s ease',
    },
    emptyState: { textAlign: 'center', padding: '3rem', color: '#999' },
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>Habitational Insurance</h1>
      ...
      <div style={styles.tableCard}>
        <div style={styles.tableHeader}>
          ...
          <span style={styles.quoteCount}>{savedHabQuotes.length} quote{savedHabQuotes.length !== 1 ? 's' : ''}</span>
        </div>
        {savedHabQuotes.length === 0 ? (
          <div style={styles.emptyState}><p>No saved quotes yet. Click "+ New Quote" to get started.</p></div>
        ) : (
          <table style={styles.table}>
            ...
            <tbody>
              {savedHabQuotes.map((quote, idx) => {
                const customerLabel = `${quote.customer?.firstName || ''} ${quote.customer?.lastName || ''}`.trim() || '—';
                return (
                  <tr
                    key={quote.id}
                    style={{ ...styles.tr, backgroundColor: hoveredRow === idx ? '#f5f7fa' : 'transparent' }}
                  ...
                  >
                    ...
                    <td style={styles.td}>{quote.riskType || '—'}</td>
                    <td style={styles.td}><span style={styles.statusBadge(quote.status)}>{quote.status}</span></td>
