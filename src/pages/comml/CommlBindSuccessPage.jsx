...
const CommlBindSuccessPage = () => {
  ...
  const styles = {
    pageContainer: { maxWidth: '800px', margin: '0 auto', padding: '40px 24px', minHeight: 'calc(100vh - 100px)', backgroundColor: colors.white },
    successBanner: { backgroundColor: colors.successBg, border: `2px solid ${colors.successBorder}`, borderRadius: '4px', padding: '32px', textAlign: 'center', marginBottom: '32px' },
    checkmark: { width: '64px', height: '64px', borderRadius: '50%', backgroundColor: colors.success, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' },
    successTitle: { fontSize: '28px', fontWeight: 700, color: colors.success, marginBottom: '8px' },
    successSubtitle: { fontSize: '15px', color: colors.text },
    detailsBox: { border: `1px solid ${colors.border}`, borderRadius: '4px', padding: '24px', marginBottom: '24px' },
    ...
    detailLabel: { fontSize: '12px', color: '#666', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.3px', marginBottom: '4px' },
    detailValue: { fontSize: '15px', fontWeight: 600, color: colors.text },
    policyNumber: { fontSize: '24px', fontWeight: 700, color: colors.navy, letterSpacing: '1px' },
    buttonContainer: { display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '40px' },
    ...
    <div style={styles.pageContainer}>
      ...
      <div style={styles.successBanner}>
        <div style={styles.checkmark}>
          <svg style={{ width: '32px', height: '32px', fill: 'none', stroke: colors.white, strokeWidth: 3, strokeLinecap: 'round', strokeLinejoin: 'round' }} viewBox="0 0 24 24">
            <polyline points="20 6 9 17 4 12" />
          </svg>
        </div>
        <div style={styles.successTitle}>Policy Bound Successfully</div>
        <div style={styles.successSubtitle}>{bindResponse?.message || 'The policy has been bound and is now active.'}</div>
      </div>
      ...
      {bindResponse?.bindMessages?.length > 0 && (
        <div style={{
          border: '1px solid #d4a017', borderRadius: '4px', padding: '24px',
          marginBottom: '24px', backgroundColor: '#fffbf0',
        }}>
          <div style={{
            fontSize: '16px', fontWeight: 700, color: colors.navy,
            marginBottom: '16px', paddingBottom: '8px', borderBottom: '1px solid #d4a017',
          }}>
            Bind Messages
          </div>
      ...
      <div style={styles.buttonContainer}>
        <button onClick={goHome} style={styles.button}>Return to Dashboard</button>
      </div>
    </div>
  );
};
